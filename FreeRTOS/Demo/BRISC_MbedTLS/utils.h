...
#define INTERRUPT_REASON_UART1 4
...
typedef struct InterruptConfig
{
	...
	volatile uint32_t enbdev;
	volatile uint32_t pendev;
} InterruptConfig_t;

typedef struct AllocationCounter
{
	volatile uint32_t status;
	volatile uint32_t maxmem;
	volatile uint32_t curmem;
	volatile uint32_t maxhan;
	volatile uint32_t curhan;
} AllocationCounter_t;
...
extern void **io;
extern QueueHandle_t xRxedPackets;
extern QueueHandle_t xTxedPackets;
